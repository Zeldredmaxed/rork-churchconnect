...
export default function BlockedAccountsScreen() {
  ...
  const blockedQuery = useQuery({
    ...
    queryFn: async () => {
      try {
        const data = await settingsApi.getBlocked();
        console.log('[BlockedAccounts] Fetched:', data);
        return Array.isArray(data) ? data : [];
      } catch (e) {
        console.log('[BlockedAccounts] Error:', e);
  ...
  const unblockMutation = useMutation({
    ...
    onError: (err) => {
      console.log('[BlockedAccounts] Unblock error:', err);
      Alert.alert('Error', 'Failed to unblock user. Please try again.');
    },
  });

  const handleUnblock = (user: SocialBoundaryUser) => {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Unblock', style: 'destructive', onPress: () => unblockMutation.mutate(userId) },
      ]
    );
  };
  ...
  return (
    <View style={s.container}>
      ...
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={blockedQuery.isRefetching} onRefresh={handleRefresh} tintColor={theme.colors.accent} />
        }
        ListEmptyComponent={
...
const createStyles = (theme: AppTheme) => StyleSheet.create({
  ...
  unblockBtn: { paddingHorizontal: 16, paddingVertical: 7, borderRadius: 8, backgroundColor: theme.colors.surfaceElevated, borderWidth: 1, borderColor: theme.colors.border },
  unblockText: { fontSize: 13, fontWeight: '600' as const, color: theme.colors.text },
});
