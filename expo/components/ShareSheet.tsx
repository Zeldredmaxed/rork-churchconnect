...
function ContactItem({ user, isSelected, isSent, isSending, onSelect }: ContactItemProps) {
...
export default function ShareSheet({ visible, onClose, content }: ShareSheetProps) {
  ...
  const searchUsersQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const data = await api.get<{ data: FlockUser[] }>(`/members/search?q=${encodeURIComponent(searchQuery.trim())}`);
        return data;
      } catch {
        return { data: [] as FlockUser[] };
      }
    ...
    for (const userId of usersToSend) {
      try {
        const shareMessage = message.trim()
          ? `${message.trim()}\n\nShared ${content.type}: "${content.title}"`
          : `Shared a ${content.type}: "${content.title}"`;

        await api.post('/chat/send', {
          recipient_id: userId,
          content: shareMessage,
          shared_content_id: content.id,
          shared_content_type: content.type,
        });
        console.log('[Share] Sent to user:', userId);
        setSentUsers((prev) => [...prev, userId]);
      } catch (error) {
        console.log('[Share] Failed to send to user:', userId, error);
      }
  ...
  const handleCopyLink = useCallback(async () => {
    ...
    try {
      ...
      console.log('[Share] Failed to copy link');
    }
  }, [content]);
  ...
  const handleExternalShare = useCallback(async () => {
    ...
    try {
      await Share.share({
        message: content.title ? `Check this out: ${content.title}\n${link}` : link,
        url: Platform.OS === 'ios' ? link : undefined,
      });
    } catch {
      console.log('[Share] External share cancelled or failed');
    }
  }, [content]);
      ...
      <View style={styles.overlay}>
        ...
        <Animated.View style={[styles.sheet, { transform: [{ translateY }] }]}>
            ...
            {contactsQuery.isLoading && !isSearching ? (
              ...
              <View style={styles.contactsEmpty}>
                <Text style={styles.contactsEmptyText}>
                  {isSearching ? 'No results found' : 'No contacts available'}
                </Text>
              </View>
            ) : (
              <FlatList
                ...
                renderItem={({ item }) => (
                  <ContactItem
                    ...
                    isSent={sentUsers.includes(item.id)}
                    isSending={sendingUsers.includes(item.id)}
            ...
            <View style={styles.messageSection}>
              <View style={styles.messageInputRow}>
                ...
                <TouchableOpacity
                  style={[
                    styles.sendBtn,
                    !hasSelectedUnsent && styles.sendBtnDisabled,
                  ]}
                  onPress={handleSend}
                  disabled={!hasSelectedUnsent || sendingUsers.length > 0}
                  activeOpacity={0.7}
                >
                  {sendingUsers.length > 0 ? (
                    ...
                    <Text style={styles.sendBtnText}>
                      {sentUsers.length > 0 && !hasSelectedUnsent ? 'Sent' : 'Send'}
                    </Text>
                  )}
                </TouchableOpacity>
              </View>
          ...
          <View style={styles.actionsSection}>
            <ActionButton
              icon={<Link2 size={22} color={theme.colors.text} />}
              label="Copy link"
              onPress={handleCopyLink}
            />
            <ActionButton
              icon={<Share2 size={22} color={theme.colors.text} />}
              label="Share"
              onPress={handleExternalShare}
            />
            <ActionButton
              icon={<MessageCircle size={22} color={theme.colors.text} />}
              label="Send in chat"
              onPress={handleSendInChat}
            />
            <ActionButton
              icon={<PlusCircle size={22} color={theme.colors.text} />}
              label="Add to story"
              onPress={() => {
                void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                Alert.alert('Coming Soon', 'Stories feature is coming soon!');
              }}
            />
          </View>
        </Animated.View>
