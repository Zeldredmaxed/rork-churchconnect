...
export default function MentionInput({
  ...
  const membersQuery = useQuery({
    ...
    queryFn: () =>
      api.get<MemberSearchResult | Member[]>(
        `/members?search=${encodeURIComponent(mentionQuery ?? '')}&per_page=8`
      ),
  ...
  const renderMemberItem = useCallback(
    ({ item }: { item: Member }) => {
      ...
      return (
        <TouchableOpacity
          style={[suggestionStyles.memberRow, darkMode && suggestionStyles.memberRowDark]}
          onPress={() => handleSelectMember(item)}
          activeOpacity={0.6}
        >
          <View style={[suggestionStyles.memberAvatar, darkMode && suggestionStyles.memberAvatarDark]}>
            <Text style={[suggestionStyles.memberAvatarText, darkMode && suggestionStyles.memberAvatarTextDark]}>
              {initials}
            </Text>
          </View>
          <View style={suggestionStyles.memberInfo}>
            <Text style={[suggestionStyles.memberName, darkMode && suggestionStyles.memberNameDark]} numberOfLines={1}>
              {item.full_name}
            </Text>
            <Text style={suggestionStyles.memberEmail} numberOfLines={1}>
              {item.email}
            </Text>
          </View>
        </TouchableOpacity>
  ...
  const suggestionsView = showSuggestions ? (
    <Animated.View
      style={[
        ...
        darkMode && suggestionStyles.containerDark,
        suggestionsPosition === 'above' ? suggestionStyles.above : suggestionStyles.below,
        { opacity: fadeAnim },
      ]}
    >
      <View style={[suggestionStyles.header, darkMode && suggestionStyles.headerDark]}>
        ...
        <Text style={[suggestionStyles.headerText, darkMode && suggestionStyles.headerTextDark]}>
          Mention a member
        </Text>
        <TouchableOpacity
          onPress={() => {
      ...
      {membersQuery.isLoading ? (
        ...
        <View style={suggestionStyles.emptyContainer}>
          <Text style={[suggestionStyles.emptyText, darkMode && suggestionStyles.emptyTextDark]}>
            No members found
          </Text>
        </View>
      ) : (
  ...
  return (
    <View style={[mentionStyles.wrapper, containerStyle]}>
      ...
      <TextInput
        ...
        style={[mentionStyles.input, inputStyle]}
        value={value}
