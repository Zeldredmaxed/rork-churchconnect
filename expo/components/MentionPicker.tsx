...
interface MemberSearchResult { data?: Member[]; }
...
export default function MentionPicker({ visible, onClose, onSelect, selectedIds = [], multiSelect = true }: MentionPickerProps) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      Animated.spring(slideAnim, { toValue: 0, useNativeDriver: true, friction: 9, tension: 65 }).start(() => { searchInputRef.current?.focus(); });
    } else { slideAnim.setValue(SHEET_HEIGHT); setSearch(''); }
  }, [visible, slideAnim, selectedIds]);

  const handleClose = useCallback(() => {
    Animated.timing(slideAnim, { toValue: SHEET_HEIGHT, duration: 250, useNativeDriver: true }).start(() => { onClose(); });
  }, [onClose, slideAnim]);
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_evt, gs) => { if (gs.dy > 0) slideAnim.setValue(gs.dy); },
      onPanResponderRelease: (_evt, gs) => {
        if (gs.dy > 80 || gs.vy > 0.5) { handleClose(); }
        else { Animated.spring(slideAnim, { toValue: 0, useNativeDriver: true, friction: 9 }).start(); }
      },
    })
  ).current;
  ...
  const renderMember = useCallback(({ item }: { item: Member }) => {
    ...
    return (
      <TouchableOpacity style={pickerStyles.memberRow} onPress={() => toggleMember(item)} activeOpacity={0.6}>
        <View style={pickerStyles.memberAvatar}><Text style={pickerStyles.memberAvatarText}>{initials}</Text></View>
  ...
  }, [selected, toggleMember, pickerStyles, theme]);

  return (
    <Modal visible={visible} transparent animationType="none" onRequestClose={handleClose}>
      <View style={pickerStyles.backdrop}>
        <TouchableOpacity style={pickerStyles.backdropTouch} activeOpacity={1} onPress={handleClose} />
        <Animated.View style={[pickerStyles.sheet, { transform: [{ translateY: slideAnim }], paddingBottom: insets.bottom }]} {...panResponder.panHandlers}>
          <View style={pickerStyles.handleBar}><View style={pickerStyles.handle} /></View>
          <View style={pickerStyles.searchContainer}>
            <View style={pickerStyles.searchRow}>
              <Search size={18} color={theme.colors.textTertiary} />
              <TextInput ref={searchInputRef} style={pickerStyles.searchInput} value={search} onChangeText={setSearch} placeholder="Search" placeholderTextColor={theme.colors.textTertiary} autoCapitalize="none" autoCorrect={false} />
              {search.length > 0 && (<TouchableOpacity onPress={() => setSearch('')} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><X size={16} color={theme.colors.textTertiary} /></TouchableOpacity>)}
            </View>
          </View>
          {membersQuery.isLoading ? (
            <View style={pickerStyles.loadingContainer}><ActivityIndicator size="small" color={theme.colors.accent} /></View>
          ) : members.length === 0 ? (
          ...
          <View style={pickerStyles.footer}>
            <TouchableOpacity style={[pickerStyles.addBtn, selected.size === 0 && pickerStyles.addBtnDisabled]} onPress={handleAdd} disabled={selected.size === 0} activeOpacity={0.7}>
              <Text style={[pickerStyles.addBtnText, selected.size === 0 && pickerStyles.addBtnTextDisabled]}>Add{selected.size > 0 ? ` (${selected.size})` : ''}</Text>
            </TouchableOpacity>
          </View>
        </Animated.View>
      </View>
...
const createPickerStyles = (theme: AppTheme) => StyleSheet.create({
  ...
  sheet: { backgroundColor: theme.colors.surface, borderTopLeftRadius: 16, borderTopRightRadius: 16, maxHeight: SHEET_HEIGHT, minHeight: 320 },
  handleBar: { alignItems: 'center', paddingTop: 8, paddingBottom: 4 },
  handle: { width: 36, height: 4, borderRadius: 2, backgroundColor: theme.colors.border },
  searchContainer: { paddingHorizontal: 14, paddingVertical: 8 },
  searchRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: theme.colors.surfaceElevated, borderRadius: 10, paddingHorizontal: 12, height: 40, gap: 8, borderWidth: 1, borderColor: theme.colors.border },
  ...
  memberRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 10, gap: 12 },
  memberAvatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: theme.colors.accentMuted, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: theme.colors.border },
  memberAvatarText: { fontSize: 14, fontWeight: '700' as const, color: theme.colors.accent },
  memberInfo: { flex: 1, gap: 2 },
  memberName: { fontSize: 15, fontWeight: '600' as const, color: theme.colors.text },
  memberEmail: { fontSize: 13, color: theme.colors.textTertiary },
  ...
  footer: { paddingHorizontal: 16, paddingTop: 10, paddingBottom: 8, borderTopWidth: 0.5, borderTopColor: theme.colors.borderLight },
  addBtn: { backgroundColor: theme.colors.accent, borderRadius: 10, paddingVertical: 14, alignItems: 'center' },
  addBtnDisabled: { backgroundColor: theme.colors.surfaceElevated },
  addBtnText: { fontSize: 16, fontWeight: '600' as const, color: theme.colors.white },
  addBtnTextDisabled: { color: theme.colors.textTertiary },
});
